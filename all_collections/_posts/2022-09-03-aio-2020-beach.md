...
Note that if `a % X > b % X`, the range will "wrap around", we express this as `[a % X, X-1]` $${\displaystyle \cup}$$ `[0, b % X]`. For example, in the image above [5, 2] is `[5, 5]` $${\displaystyle \cup}$$ `[0, 2]`.
...
**Optimally we want to keep going `i->fsu[i]` as far as possible, as each "skip" gives as some extra space.** We want to ascend the child->ancestor path as far as possible without exceeding the cost limit of K spare umbrellas. To do this, we binary search on the path from `i` to the root of the tree.

Remember that the nodes in the forest are our starting points right after pre-placed umbrellas.

Define the furthest node we can reach from i to be `ans[i]`. It is possible that there are leftover umbrellas which we need to place. We don't need to apply the skipping process to these umbrellas. Let the # of spaces before umbrella `i` be `l`, the # of spaces after `ans[i]` be r. Greedily assign umbrellas to `l` and `r`. (Be careful, placing an umbrella does not necessarily mean you get X extra spots).

One final edge case: in an optimal solution the skipping process might be useless in which case the answer is `min(L, K * X)`.

Proof and implementation is left as an exercise to the reader. (Hint: it's painful) 